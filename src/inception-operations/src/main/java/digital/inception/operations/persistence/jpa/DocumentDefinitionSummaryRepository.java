...
/**
 * The {@code DocumentDefinitionSummaryRepository} interface declares the persistence for the {@code
 * DocumentDefinitionSummary} domain type.
 *
...
public interface DocumentDefinitionSummaryRepository
  ...
  @Query(
      """
         select dd
           from DocumentDefinitionSummary dd
          where dd.categoryId = :categoryId
             and (dd.tenantId = :tenantId or dd.tenantId is null)
          order by dd.id
         """)
  List<DocumentDefinitionSummary> findForCategoryAndTenantOrGlobal(
