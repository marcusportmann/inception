...
EXEC sys.sp_addextendedproperty
@name=N'MS_Description', @value=N'The username for the user associated with the password reset' ,
@level0type=N'SCHEMA', @level0name=N'SECURITY', @level1type=N'TABLE', @level1name=N'PASSWORD_RESETS', @level2type=N'COLUMN', @level2name=N'USERNAME';

EXEC sys.sp_addextendedproperty
@name=N'MS_Description', @value=N'The date and time the password reset was requested' ,
@level0type=N'SCHEMA', @level0name=N'SECURITY', @level1type=N'TABLE', @level1name=N'PASSWORD_RESETS', @level2type=N'COLUMN', @level2name=N'REQUESTED';

EXEC sys.sp_addextendedproperty
@name=N'MS_Description', @value=N'The status of the password reset' ,
...
@level0type=N'SCHEMA', @level0name=N'SECURITY', @level1type=N'TABLE', @level1name=N'PASSWORD_RESETS', @level2type=N'COLUMN', @level2name=N'COMPLETED';

EXEC sys.sp_addextendedproperty
@name=N'MS_Description', @value=N'The date and time the password reset expired' ,
@level0type=N'SCHEMA', @level0name=N'SECURITY', @level1type=N'TABLE', @level1name=N'PASSWORD_RESETS', @level2type=N'COLUMN', @level2name=N'EXPIRED';
GO

CREATE TRIGGER users_on_delete_password_resets_trigger
  ON "SECURITY"."USERS"
  FOR DELETE AS BEGIN DECLARE @USERNAME NVARCHAR(100) DECLARE C CURSOR FOR
SELECT deleted.username
  FROM deleted open C FETCH NEXT
  FROM C
  INTO @USERNAME WHILE @@FETCH_STATUS = 0 BEGIN
DELETE
  FROM "SECURITY"."USER_TO_GROUP_MAP"
  WHERE username = @USERNAME FETCH NEXT
  FROM C
