...
public interface IndividualCustomerRepository extends JpaRepository<IndividualCustomer, UUID> {
  ...
  @Query("select ic from IndividualCustomer ic where (lower(ic.name) like lower(:filter))")
  Page<IndividualCustomer> findFiltered(@Param("filter") String filter, Pageable pageable);
}
