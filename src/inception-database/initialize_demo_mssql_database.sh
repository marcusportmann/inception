...
function update_liquibase_changelogs {
  ...
  liquibase --changeLogFile=../inception-party/src/main/resources/db/inception-party-data.changelog.xml --username=$db_username --password=$db_password --url="jdbc:sqlserver://$db_host:$db_port;databaseName=$db_name;trustServerCertificate=true" --classpath=drivers/mssql-jdbc.jar --driver=com.microsoft.sqlserver.jdbc.SQLServerDriver update
