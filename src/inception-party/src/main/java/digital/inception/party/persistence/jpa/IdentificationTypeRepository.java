...
public interface IdentificationTypeRepository
  ...
  /**
   * Find all the identification types sorted by locale ID, sort order, and name.
   *
   * @return all the identification types sorted by locale ID, sort order, and name.
   */
  @Query(
      "select idt from IdentificationType idt order by idt.localeId, idt.sortOrder DESC, idt.name")
