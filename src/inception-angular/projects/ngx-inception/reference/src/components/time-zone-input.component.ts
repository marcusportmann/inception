...
export class TimeZoneInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<TimeZone[]> = new BehaviorSubject<TimeZone[]>([]);
  ...
  ngOnInit(): void {
    ...
    this.referenceService.getTimeZones().pipe(first()).subscribe((timeZones: Map<string, TimeZone>) => {
