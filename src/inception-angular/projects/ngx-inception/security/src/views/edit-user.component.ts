  ...
  implements AfterViewInit
{
  ...
  constructor(
    ...
    this.preferredNameControl = new FormControl('', [
      Validators.maxLength(100)
    ]);
    // eslint-disable-next-line max-len
    ...
    combineLatest([
      ...
      .subscribe(
        (results: [UserDirectoryCapabilities, User]) => {
          ...
          this.emailControl.setValue(results[1].email);
          this.nameControl.setValue(results[1].name);
          this.preferredNameControl.setValue(results[1].preferredName);
          this.mobileNumberControl.setValue(results[1].mobileNumber);
          this.phoneNumberControl.setValue(results[1].phoneNumber);
  ...
  ok(): void {
    if (this.user && this.editUserForm.valid) {
      ...
      const expirePasswordFormControl: AbstractControl | null =
        this.editUserForm.get('expirePassword');
      const lockUserFormControl: AbstractControl | null =
        this.editUserForm.get('lockUser');

      this.securityService
        .updateUser(
          this.user,
          expirePasswordFormControl ? expirePasswordFormControl.value : false,
