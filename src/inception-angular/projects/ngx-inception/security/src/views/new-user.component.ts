  ...
  ok(): void {
    if (this.user && this.newUserForm.valid) {
      ...
      this.securityService.createUser(this.user,
        this.newUserForm.contains('expiredPassword') ? this.expiredPasswordControl.value : false,
        this.newUserForm.contains('userLocked') ? this.userLockedControl.value : false)
      .pipe(first(), finalize(() => this.spinnerService.hideSpinner()))
      .subscribe(() => {
        // noinspection JSIgnoredPromiseFromCall
        this.router.navigate(['../..'], {
