...
export class UserProfileComponent extends AdminContainerView implements OnInit {
  ...
  readonly mobileNumberControl: FormControl<string | null>;

  readonly nameControl: FormControl<string>;

  readonly phoneNumberControl: FormControl<string | null>;

  readonly preferredNameControl: FormControl<string | null>;

  readonly title = $localize`:@@security_user_profile_title:Profile`;

  user: User | null = null;

  readonly userProfileForm: FormGroup<{
  ...
  constructor() {
    ...
    this.preferredNameControl = new FormControl<string | null>(null, [Validators.maxLength(100)]);

    this.mobileNumberControl = new FormControl<string | null>(null, [
      Validators.maxLength(100),
      Validators.pattern(
  ...
  copyAccessToken(): void {
    // Read once, copy, and forget — never touches the template.
