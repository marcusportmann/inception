...
export class ExternalReferenceTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<ExternalReferenceType[]> = new BehaviorSubject<ExternalReferenceType[]>([]);
  ...
  /**
   * The external reference type options.
   */
  private _options: ExternalReferenceType[] = [];

  /**
   * The code for the party type to retrieve the external reference types for.
   */
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.partyType$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getExternalReferenceTypes().pipe(first()).subscribe((externalReferenceTypes: Map<string, ExternalReferenceType>) => {
        ...
        for (const externalReferenceType of externalReferenceTypes.values()) {
          if ((!parameters.partyType) || ((!!externalReferenceType.partyTypes) && (externalReferenceType.partyTypes.indexOf(parameters.partyType) !== -1))) {
            this._options.push(externalReferenceType);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: ExternalReferenceType[] = [];
