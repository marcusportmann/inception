...
/**
 * The GenderInputComponent class implements the gender input component.
 *
  ...
  template: `
    <div matAutocompleteOrigin #origin="matAutocompleteOrigin">
      <input
        ...
        required="required"
        [matAutocomplete]="genderAutocomplete"
        [matAutocompleteConnectedTo]="origin"
        (input)="genderInputChanged($event)"
        (focusin)="onFocusIn($event)"
        (focusout)="onFocusOut($event)">
      <mat-autocomplete
        #genderAutocomplete="matAutocomplete"
        [displayWith]="displayGender"
...
export class GenderInputComponent implements MatFormFieldControl<string>,
  ...
  /**
   * The code for the selected gender.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected gender.
   *
   * @return The code for the selected gender.
   */
  ...
  /**
   * Set the code for the selected gender.
   *
   * @param value the code for the selected gender
   */
  @Input()
        ...
        if (!!value) {
          for (const gender of genders.values()) {
            if (gender.code === value) {
              this._value = value;
              this.genderInput.value = gender.name;
              break;
            }
          }
        }
  ...
  genderInputChanged(event: Event) {
  ...
  ngOnInit(): void {
    this.genderInput.placeholder = this._placeholder;

    this.partyReferenceService.getGenders().pipe(first()).subscribe((genders: Map<string, Gender>) => {
      this.subscriptions.add(this.genderInputValue$.pipe(
        startWith(''),
        debounceTime(500)).subscribe((value: string | Gender) => {
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if ((!!this._value) && (!this.genderInput.value)) {
      this._value = null;
      this.onChange(this._value);
      this.changeDetectorRef.detectChanges();
    }
    ...
    this.touched = true;
    this.onTouched();
    this.focused = this.genderInput.focused;
    this.stateChanges.next();
  }
  ...
  selectGender(event: MatAutocompleteSelectedEvent): void {
