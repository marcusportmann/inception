...
/**
 * The IndustryClassificationSystemInputComponent class implements the industry classification system input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'industry-classification-system-input',
  template: `
    <div matAutocompleteOrigin #origin="matAutocompleteOrigin">
      <input
        #industryClassificationSystemInput
        type="text"
        matInput
        autocompleteSelectionRequired
        required="required"
        [matAutocomplete]="industryClassificationSystemAutocomplete"
        [matAutocompleteConnectedTo]="origin"
        (input)="industryClassificationSystemInputChanged($event)"
        (focusin)="onFocusIn($event)"
        (focusout)="onFocusOut($event)">
      <mat-autocomplete
        #industryClassificationSystemAutocomplete="matAutocomplete"
        [displayWith]="displayIndustryClassificationSystem"
        (optionSelected)="selectIndustryClassificationSystem($event)">
        <mat-option
          *ngFor="let industryClassificationSystem of filteredIndustryClassificationSystems$ | async"
          [value]="industryClassificationSystem">
  ...
  providers: [
    {
      ...
      useExisting: IndustryClassificationSystemInputComponent
    }
  ]
})
export class IndustryClassificationSystemInputComponent implements MatFormFieldControl<string>,
  ...
  /**
   * The reference to the element for the industry classification system input.
   */
  @ViewChild('industryClassificationSystemInput') industryClassificationSystemInputElementRef!: ElementRef;
  ...
  /**
   * The placeholder for the industry classification system input.
   * @private
   */
  ...
  /**
   * The code for the selected industry classification system.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected industry classification system.
   *
   * @return The code for the selected industry classification system.
   */
  ...
  /**
   * Set the code for the selected industry classification system.
   *
   * @param value the code for the selected industry classification system
   */
  @Input()
  ...
  industryClassificationSystemInputChanged(event: Event) {
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getIndustryClassificationSystems().pipe(first()).subscribe((industryClassificationSystems: Map<string, IndustryClassificationSystem>) => {
      this.subscriptions.add(this.industryClassificationSystemInputValue$.pipe(
        startWith(''),
        debounceTime(500)).subscribe((value: string | IndustryClassificationSystem) => {
  ...
  selectIndustryClassificationSystem(event: MatAutocompleteSelectedEvent): void {
