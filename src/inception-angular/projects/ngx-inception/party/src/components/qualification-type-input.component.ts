...
import {QualificationType} from '../services/qualification-type';

/**
 * The QualificationTypeInputComponent class implements the qualification type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'qualification-type-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: QualificationTypeInputComponent
    }
  ]
})
export class QualificationTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<QualificationType[]> = new BehaviorSubject<QualificationType[]>([]);
   ...
   */
  private _options: QualificationType[] = [];
  ...
  /**
   * The code for the selected qualification type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected qualification type.
   *
   * @return The code for the selected qualification type.
   */
  ...
  /**
   * Set the code for the selected qualification type.
   *
   * @param value the code for the selected qualification type
   */
  @Input()
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getQualificationTypes().pipe(first()).subscribe((qualificationTypes: Map<string, QualificationType>) => {
      this._options = Array.from(qualificationTypes.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: QualificationType[] = [];
