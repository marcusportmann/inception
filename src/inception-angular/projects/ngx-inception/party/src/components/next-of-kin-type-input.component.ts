...
export class NextOfKinTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<NextOfKinType[]> = new BehaviorSubject<NextOfKinType[]>([]);
   ...
   */
  private _options: NextOfKinType[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getNextOfKinTypes().pipe(first()).subscribe((nextOfKinTypes: Map<string, NextOfKinType>) => {
      this._options = Array.from(nextOfKinTypes.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: NextOfKinType[] = [];
