  ...
  template: `
    <div matAutocompleteOrigin #origin="matAutocompleteOrigin">
      ...
      <mat-autocomplete
        ...
        <mat-option
          *ngFor="let contactMechanismType of filteredOptions$ | async"
...
export class ContactMechanismTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<ContactMechanismType[]> = new BehaviorSubject<ContactMechanismType[]>([]);
        ...
        if (!!value) {
          for (const contactMechanismType of contactMechanismTypes.values()) {
            if (contactMechanismType.code === value) {
              this._value = value;
              this.input.value = contactMechanismType.name;
              break;
            }
          }
        }
  ...
  displayWith(contactMechanismType: ContactMechanismType): string {
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getContactMechanismTypes().pipe(first()).subscribe((contactMechanismTypes: Map<string, ContactMechanismType>) => {
      this.subscriptions.add(this.inputValue$.pipe(
        ...
        this.filteredOptions$.next(filteredContactMechanismTypes);
      }));
    });
  }
