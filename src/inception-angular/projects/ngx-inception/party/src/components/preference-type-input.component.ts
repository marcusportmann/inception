...
import {PreferenceType} from '../services/preference-type';
import {PartyReferenceService} from '../services/party-reference.service';

/**
 * The PreferenceTypeInputComponent class implements the preference type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'preference-type-input',
  template: `
    <div matAutocompleteOrigin #origin="matAutocompleteOrigin">
      <input
        ...
        required="required"
        [matAutocomplete]="preferenceTypeAutocomplete"
  ...
  providers: [
    {
      ...
      useExisting: PreferenceTypeInputComponent
    }
  ]
})
export class PreferenceTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  controlType = 'preference-type-input';

  /**
   * The filtered options for the autocomplete.
   */
  filteredOptions$: BehaviorSubject<PreferenceType[]> = new BehaviorSubject<PreferenceType[]>([]);
  ...
  /**
   * The preference type options.
   */
  private _options: PreferenceType[] = [];

  /**
   * The code for the preference type category to retrieve the preference types for.
   */
  private category$: BehaviorSubject<string | null> = new BehaviorSubject<string | null>(null);

  /**
   * The code for the party type to retrieve the preference types for.
   */
  ...
  /**
   * The code for the selected preference type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected preference type.
   *
   * @return The code for the selected preference type.
   */
  ...
  /**
   * Set the code for the selected preference type.
   *
   * @param value the code for the selected preference type
   */
  @Input()
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.category$, this.partyType$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getPreferenceTypes().pipe(first()).subscribe((preferenceTypes: Map<string, PreferenceType>) => {
        ...
        for (const preferenceType of preferenceTypes.values()) {
          if (((!parameters.category) || (preferenceType.category === parameters.category)) &&
            ((!parameters.partyType) || ((!!preferenceType.partyTypes) && (preferenceType.partyTypes.indexOf(parameters.partyType) !== -1)))) {
            this._options.push(preferenceType);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: PreferenceType[] = [];
