...
export class ResidentialTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<ResidentialType[]> = new BehaviorSubject<ResidentialType[]>([]);
   ...
   */
  private _options: ResidentialType[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getResidentialTypes().pipe(first()).subscribe((residentialTypes: Map<string, ResidentialType>) => {
      this._options = Array.from(residentialTypes.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: ResidentialType[] = [];
