...
import {ResidentialType} from '../services/residential-type';

/**
 * The ResidentialTypeInputComponent class implements the residential type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'residential-type-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: ResidentialTypeInputComponent
    }
  ]
})
export class ResidentialTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  @HostBinding() id = `residential-type-input-${ResidentialTypeInputComponent._nextId++}`;

  /**
   * The residential type input.
   */
  ...
  set disabled(value: boolean) {
    ...
    if (this._disabled) {
      this.residentialTypeInput.disabled = true;
    }
  ...
  /**
   * The code for the selected residential type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected residential type.
   *
   * @return The code for the selected residential type.
   */
  ...
  /**
   * Set the code for the selected residential type.
   *
   * @param value the code for the selected residential type
   */
  @Input()
  public set value(value: string | null) {
    ...
    if (this._value !== value) {
      this.partyReferenceService.getResidentialTypes().pipe(first()).subscribe((residentialTypes: Map<string, ResidentialType>) => {
        this._value = null;
        this.residentialTypeInput.value = '';

        if (!!value) {
          for (const residentialType of residentialTypes.values()) {
            if (residentialType.code === value) {
              this._value = value;
              this.residentialTypeInput.value = residentialType.name;
              break;
            }
          }
        }
  ...
  ngOnInit(): void {
    this.residentialTypeInput.placeholder = this._placeholder;
  ...
  onContainerClick(event: MouseEvent) {
    if ((event.target as Element).tagName.toLowerCase() != 'input') {
      this.residentialTypeInput.focus();
    }
  }
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if ((!!this._value) && (!this.residentialTypeInput.value)) {
      this._value = null;
      this.onChange(this._value);
      this.changeDetectorRef.detectChanges();
    }
    ...
    this.touched = true;
    this.onTouched();
    this.focused = this.residentialTypeInput.focused;
    this.stateChanges.next();
  }
