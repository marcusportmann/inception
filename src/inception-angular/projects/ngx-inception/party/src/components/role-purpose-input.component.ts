...
export class RolePurposeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<RolePurpose[]> = new BehaviorSubject<RolePurpose[]>([]);
        ...
        if (!!value) {
          for (const rolePurpose of rolePurposes.values()) {
            if (rolePurpose.code === value) {
              this._value = value;
              this.input.value = rolePurpose.name;
              break;
            }
          }
        }
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getRolePurposes().pipe(first()).subscribe((rolePurposes: Map<string, RolePurpose>) => {
      this.subscriptions.add(this.inputValue$.pipe(
        ...
        this.filteredOptions$.next(filteredRolePurposes);
      }));
    });
  }
