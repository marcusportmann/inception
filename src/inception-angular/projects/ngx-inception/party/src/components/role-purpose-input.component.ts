...
import {RolePurpose} from '../services/role-purpose';

/**
 * The RolePurposeInputComponent class implements the role purpose input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'role-purpose-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: RolePurposeInputComponent
    }
  ]
})
export class RolePurposeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  @HostBinding() id = `role-purpose-input-${RolePurposeInputComponent._nextId++}`;

  /**
   * The role purpose input.
   */
  @ViewChild(MatInput, {static: true}) rolePurposeInput!: MatInput;

  /**
   * The reference to the element for the role purpose input.
   */
  @ViewChild('rolePurposeInput') rolePurposeInputElementRef!: ElementRef;

  /**
   * The observable providing access to the value for the role purpose input as it changes.
   */
  rolePurposeInputValue$: Subject<string> = new ReplaySubject<string>();
  ...
  set disabled(value: boolean) {
    ...
    if (this._disabled) {
      this.rolePurposeInput.disabled = true;
    }
  ...
  /**
   * The code for the selected role purpose.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected role purpose.
   *
   * @return The code for the selected role purpose.
   */
  ...
  /**
   * Set the code for the selected role purpose.
   *
   * @param value the code for the selected role purpose
   */
  @Input()
  public set value(value: string | null) {
    ...
    if (this._value !== value) {
      this.partyReferenceService.getRolePurposes().pipe(first()).subscribe((rolePurposes: Map<string, RolePurpose>) => {
        this._value = null;
        this.rolePurposeInput.value = '';

        if (!!value) {
          for (const rolePurpose of rolePurposes.values()) {
            if (rolePurpose.code === value) {
              this._value = value;
              this.rolePurposeInput.value = rolePurpose.name;
              break;
            }
          }
        }
  ...
  ngOnInit(): void {
    this.rolePurposeInput.placeholder = this._placeholder;
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if ((!!this._value) && (!this.rolePurposeInput.value)) {
      this._value = null;
      this.onChange(this._value);
      this.changeDetectorRef.detectChanges();
    }
    ...
    this.touched = true;
    this.onTouched();
    this.focused = this.rolePurposeInput.focused;
    this.stateChanges.next();
  }
