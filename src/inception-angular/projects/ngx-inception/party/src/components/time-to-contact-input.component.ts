...
export class TimeToContactInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<TimeToContact[]> = new BehaviorSubject<TimeToContact[]>([]);
   ...
   */
  private _options: TimeToContact[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getTimesToContact().pipe(first()).subscribe((timesToContact: Map<string, TimeToContact>) => {
      this._options = Array.from(timesToContact.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: TimeToContact[] = [];
