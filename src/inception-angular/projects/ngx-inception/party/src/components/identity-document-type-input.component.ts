...
import {IdentityDocumentType} from '../services/identity-document-type';
import {PartyReferenceService} from '../services/party-reference.service';

/**
 * The IdentityDocumentTypeInputComponent class implements the identity document type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'identity-document-type-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: IdentityDocumentTypeInputComponent
    }
  ]
})
export class IdentityDocumentTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<IdentityDocumentType[]> = new BehaviorSubject<IdentityDocumentType[]>([]);
   ...
   */
  @HostBinding() id = `identity-document-type-input-${IdentityDocumentTypeInputComponent._nextId++}`;
  ...
  /**
   * The identity document type options.
   */
  private _options: IdentityDocumentType[] = [];

  /**
   * The code for the aaa bbb to retrieve the identity document types for.
   */
  ...
  /**
   * The code for the selected identity document type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected identity document type.
   *
   * @return The code for the selected identity document type.
   */
  ...
  /**
   * Set the code for the selected identity document type.
   *
   * @param value the code for the selected identity document type
   */
  @Input()
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.partyType$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getIdentityDocumentTypes().pipe(first()).subscribe((identityDocumentTypes: Map<string, IdentityDocumentType>) => {
        ...
        for (const identityDocumentType of identityDocumentTypes.values()) {
          if ((!parameters.partyType) || ((!!identityDocumentType.partyTypes) && (identityDocumentType.partyTypes.indexOf(parameters.partyType) !== -1)))
          {
            this._options.push(identityDocumentType);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      for (const identityDocumentType of this._options) {
