...
import {SegmentationType} from '../services/segmentation-type';

/**
 * The SegmentationTypeInputComponent class implements the segmentation type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'segmentation-type-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: SegmentationTypeInputComponent
    }
  ]
})
export class SegmentationTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<SegmentationType[]> = new BehaviorSubject<SegmentationType[]>([]);
  ...
  /**
   * The segmentation type options.
   */
  private _options: SegmentationType[] = [];

  /**
   * The code for the party type to retrieve the segmentation types for.
   */
  ...
  /**
   * The code for the selected segmentation type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected segmentation type.
   *
   * @return The code for the selected segmentation type.
   */
  ...
  /**
   * Set the code for the selected segmentation type.
   *
   * @param value the code for the selected segmentation type
   */
  @Input()
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.partyType$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getSegmentationTypes().pipe(first()).subscribe((segmentationTypes: Map<string, SegmentationType>) => {
        ...
        for (const segmentationType of segmentationTypes.values()) {
          if ((!parameters.partyType) || ((!!segmentationType.partyTypes) && (segmentationType.partyTypes.indexOf(parameters.partyType) !== -1))) {
            this._options.push(segmentationType);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: SegmentationType[] = [];
