...
/**
 * The PreferenceTypeCategoryInputComponent class implements the preference type category input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'preference-type-category-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: PreferenceTypeCategoryInputComponent
    }
  ]
})
export class PreferenceTypeCategoryInputComponent implements MatFormFieldControl<string>,
   ...
   */
  @HostBinding() id = `preference-type-category-input-${PreferenceTypeCategoryInputComponent._nextId++}`;

  /**
   * The preference type category input.
   */
  @ViewChild(MatInput, {static: true}) preferenceTypeCategoryInput!: MatInput;

  /**
   * The reference to the element for the preference type category input.
   */
  @ViewChild('preferenceTypeCategoryInput') preferenceTypeCategoryInputElementRef!: ElementRef;

  /**
   * The observable providing access to the value for the preference type category input as it changes.
   */
  preferenceTypeCategoryInputValue$: Subject<string> = new ReplaySubject<string>();
  ...
  set disabled(value: boolean) {
    ...
    if (this._disabled) {
      this.preferenceTypeCategoryInput.disabled = true;
    }
  ...
  /**
   * The code for the selected preference type category.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected preference type category.
   *
   * @return The code for the selected preference type category.
   */
  ...
  /**
   * Set the code for the selected preference type category.
   *
   * @param value the code for the selected preference type category
   */
  @Input()
  public set value(value: string | null) {
    ...
    if (this._value !== value) {
      this.partyReferenceService.getPreferenceTypeCategories().pipe(first()).subscribe((preferenceTypeCategories: Map<string, PreferenceTypeCategory>) => {
        this._value = null;
        this.preferenceTypeCategoryInput.value = '';

        if (!!value) {
          for (const preferenceTypeCategory of preferenceTypeCategories.values()) {
            if (preferenceTypeCategory.code === value) {
              this._value = value;
              this.preferenceTypeCategoryInput.value = preferenceTypeCategory.name;
              break;
            }
          }
        }
  ...
  ngOnInit(): void {
    this.preferenceTypeCategoryInput.placeholder = this._placeholder;
  ...
  onContainerClick(event: MouseEvent) {
    if ((event.target as Element).tagName.toLowerCase() != 'input') {
      this.preferenceTypeCategoryInput.focus();
    }
  }
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if ((!!this._value) && (!this.preferenceTypeCategoryInput.value)) {
      this._value = null;
      this.onChange(this._value);
      this.changeDetectorRef.detectChanges();
    }
    ...
    this.touched = true;
    this.onTouched();
    this.focused = this.preferenceTypeCategoryInput.focused;
    this.stateChanges.next();
  }
