...
import {ResidencePermitType} from '../services/residence-permit-type';
import {PartyReferenceService} from '../services/party-reference.service';

/**
 * The ResidencePermitTypeInputComponent class implements the residence permit type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'residence-permit-type-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: ResidencePermitTypeInputComponent
    }
  ]
})
export class ResidencePermitTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<ResidencePermitType[]> = new BehaviorSubject<ResidencePermitType[]>([]);
   ...
   */
  @HostBinding() id = `residence-permit-type-input-${ResidencePermitTypeInputComponent._nextId++}`;
  ...
  /**
   * The residence permit type options.
   */
  private _options: ResidencePermitType[] = [];

  /**
   * The code for the country of issue to retrieve the residence permit types for.
   */
  ...
  /**
   * The code for the selected residence permit type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected residence permit type.
   *
   * @return The code for the selected residence permit type.
   */
  ...
  /**
   * Set the code for the selected residence permit type.
   *
   * @param value the code for the selected residence permit type
   */
  @Input()
  ...
  displayWith(residencePermitType: ResidencePermitType): string {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.countryOfIssue$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getResidencePermitTypes().pipe(first()).subscribe((residencePermitTypes: Map<string, ResidencePermitType>) => {
        ...
        for (const residencePermitType of residencePermitTypes.values()) {
          if ((!parameters.countryOfIssue) || (residencePermitType.countryOfIssue === parameters.countryOfIssue)) {
            this._options.push(residencePermitType);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: ResidencePermitType[] = [];
