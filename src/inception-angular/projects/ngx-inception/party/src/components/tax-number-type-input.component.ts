...
@Component({
  ...
  selector: 'tax-number-type-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: TaxNumberTypeInputComponent
    }
  ]
})
export class TaxNumberTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<TaxNumberType[]> = new BehaviorSubject<TaxNumberType[]>([]);
  ...
  /**
   * The code for the selected tax number type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected tax number type.
   *
   * @return The code for the selected tax number type.
   */
  ...
  /**
   * Set the code for the selected tax number type.
   *
   * @param value the code for the selected tax number type
   */
  @Input()
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: TaxNumberType[] = [];
