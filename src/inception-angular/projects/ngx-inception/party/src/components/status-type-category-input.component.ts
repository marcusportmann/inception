...
export class StatusTypeCategoryInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<StatusTypeCategory[]> = new BehaviorSubject<StatusTypeCategory[]>([]);
  ...
  /**
   * The status type category options.
   */
  private _options: StatusTypeCategory[] = [];

  /**
   * The code for the party type to retrieve the status type categorys for.
   */
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.partyType$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getStatusTypeCategories().pipe(first()).subscribe((statusTypeCategories: Map<string, StatusTypeCategory>) => {
        ...
        for (const statusTypeCategory of statusTypeCategories.values()) {
          if ((!parameters.partyType) || ((!!statusTypeCategory.partyTypes) && (statusTypeCategory.partyTypes.indexOf(parameters.partyType) !== -1))) {
            this._options.push(statusTypeCategory);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: StatusTypeCategory[] = [];
