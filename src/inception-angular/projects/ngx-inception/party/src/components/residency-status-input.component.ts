...
export class ResidencyStatusInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<ResidencyStatus[]> = new BehaviorSubject<ResidencyStatus[]>([]);
   ...
   */
  private _options: ResidencyStatus[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getResidencyStatuses().pipe(first()).subscribe((residencyStatuses: Map<string, ResidencyStatus>) => {
      this._options = Array.from(residencyStatuses.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: ResidencyStatus[] = [];
