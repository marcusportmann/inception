...
import {ResidencyStatus} from '../services/residency-status';

/**
 * The ResidencyStatusInputComponent class implements the residency status input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'residency-status-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: ResidencyStatusInputComponent
    }
  ]
})
export class ResidencyStatusInputComponent implements MatFormFieldControl<string>,
   ...
   */
  @HostBinding() id = `residency-status-input-${ResidencyStatusInputComponent._nextId++}`;

  /**
   * The residency status input.
   */
  ...
  set disabled(value: boolean) {
    ...
    if (this._disabled) {
      this.residencyStatusInput.disabled = true;
    }
  ...
  /**
   * The code for the selected residency status.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected residency status.
   *
   * @return The code for the selected residency status.
   */
  ...
  /**
   * Set the code for the selected residency status.
   *
   * @param value the code for the selected residency status
   */
  @Input()
  public set value(value: string | null) {
    ...
    if (this._value !== value) {
      this.partyReferenceService.getResidencyStatuses().pipe(first()).subscribe((residencyStatuses: Map<string, ResidencyStatus>) => {
        this._value = null;
        this.residencyStatusInput.value = '';

        if (!!value) {
          for (const residencyStatus of residencyStatuses.values()) {
            if (residencyStatus.code === value) {
              this._value = value;
              this.residencyStatusInput.value = residencyStatus.name;
              break;
            }
          }
        }
  ...
  displayWith(residencyStatus: ResidencyStatus): string {
    if (!!residencyStatus) {
  ...
  ngOnInit(): void {
    this.residencyStatusInput.placeholder = this._placeholder;
  ...
  onContainerClick(event: MouseEvent) {
    if ((event.target as Element).tagName.toLowerCase() != 'input') {
      this.residencyStatusInput.focus();
    }
  }
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if ((!!this._value) && (!this.residencyStatusInput.value)) {
      this._value = null;
      this.onChange(this._value);
      this.changeDetectorRef.detectChanges();
    }
    ...
    this.touched = true;
    this.onTouched();
    this.focused = this.residencyStatusInput.focused;
    this.stateChanges.next();
  }
