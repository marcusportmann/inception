...
import {MaritalStatus} from '../services/marital-status';
import {PartyReferenceService} from '../services/party-reference.service';

/**
 * The MaritalStatusInputComponent class implements the marital status input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'marital-status-input',
  template: `
...
export class MaritalStatusInputComponent implements MatFormFieldControl<string>,
   ...
   */
  @HostBinding() id = `marital-status-input-${MaritalStatusInputComponent._nextId++}`;

  /**
   * The marital status input.
   */
  ...
  set disabled(value: boolean) {
    ...
    if (this._disabled) {
      this.maritalStatusInput.disabled = true;
    }
  ...
  /**
   * The code for the selected marital status.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected marital status.
   *
   * @return The code for the selected marital status.
   */
  ...
  /**
   * Set the code for the selected marital status.
   *
   * @param value the code for the selected marital status
   */
  @Input()
  public set value(value: string | null) {
    ...
    if (this._value !== value) {
      this.partyReferenceService.getMaritalStatuses().pipe(first()).subscribe((maritalStatuses: Map<string, MaritalStatus>) => {
        this._value = null;
        this.maritalStatusInput.value = '';

        if (!!value) {
          for (const maritalStatus of maritalStatuses.values()) {
            if (maritalStatus.code === value) {
              this._value = value;
              this.maritalStatusInput.value = maritalStatus.name;
              break;
            }
          }
        }
  ...
  ngOnInit(): void {
    this.maritalStatusInput.placeholder = this._placeholder;
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if ((!!this._value) && (!this.maritalStatusInput.value)) {
      this._value = null;
      this.onChange(this._value);
      this.changeDetectorRef.detectChanges();
    }
    ...
    this.touched = true;
    this.onTouched();
    this.focused = this.maritalStatusInput.focused;
    this.stateChanges.next();
  }
