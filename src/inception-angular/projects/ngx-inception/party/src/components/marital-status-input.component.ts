...
export class MaritalStatusInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<MaritalStatus[]> = new BehaviorSubject<MaritalStatus[]>([]);
   ...
   */
  private _options: MaritalStatus[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getMaritalStatuses().pipe(first()).subscribe((maritalStatuses: Map<string, MaritalStatus>) => {
      this._options = Array.from(maritalStatuses.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: MaritalStatus[] = [];
