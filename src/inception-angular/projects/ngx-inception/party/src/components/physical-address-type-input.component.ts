...
export class PhysicalAddressTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<PhysicalAddressType[]> = new BehaviorSubject<PhysicalAddressType[]>([]);
   ...
   */
  private _options: PhysicalAddressType[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getPhysicalAddressTypes().pipe(first()).subscribe((physicalAddressTypes: Map<string, PhysicalAddressType>) => {
      this._options = Array.from(physicalAddressTypes.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: PhysicalAddressType[] = [];
