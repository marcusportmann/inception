    ...
    throws UsernameNotFoundException
  {
    try
    {
      ...
      else
      {
        ...
        List<String> groupNames = securityService.getGroupNamesForUser(userDirectoryId, username);

        List<String> functionCodes = securityService.getFunctionCodesForUser(userDirectoryId,
            username);

        return new digital.inception.rs.oauth.UserDetails(user, groupNames, functionCodes);
      }
    }
