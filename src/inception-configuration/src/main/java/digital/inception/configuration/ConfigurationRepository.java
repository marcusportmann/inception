...
public interface ConfigurationRepository extends JpaRepository<Configuration, String>
{
  ...
  @Query("select value from Configuration c where upper(c.key) like upper(?1) order by c.key")
  List<Configuration> getFilteredConfigurations(String filter);

  @Query("select value from Configuration c where upper(c.key) = upper(?1) order by c.key")
