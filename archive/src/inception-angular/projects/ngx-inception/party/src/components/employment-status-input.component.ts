...
export class EmploymentStatusInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<EmploymentStatus[]> = new BehaviorSubject<EmploymentStatus[]>([]);
   ...
   */
  private _options: EmploymentStatus[] = [];
  ...
  ngOnInit(): void {
    ...
    this.partyReferenceService.getEmploymentStatuses().pipe(first()).subscribe((employmentStatuses: Map<string, EmploymentStatus>) => {
      this._options = Array.from(employmentStatuses.values());
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: EmploymentStatus[] = [];
