...
import {IndustryClassificationCategory} from '../services/industry-classification-category';
import {PartyReferenceService} from '../services/party-reference.service';

/**
 * The IndustryClassificationCategoryInputComponent class implements the industry classification category input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'industry-classification-category-input',
  template: `
  ...
  providers: [
    {
      ...
      useExisting: IndustryClassificationCategoryInputComponent
    }
  ]
})
export class IndustryClassificationCategoryInputComponent implements MatFormFieldControl<string>,
   ...
   */
  controlType = 'industry-classification-category-input';

  /**
   * The filtered options for the autocomplete.
   */
  filteredOptions$: BehaviorSubject<IndustryClassificationCategory[]> = new BehaviorSubject<IndustryClassificationCategory[]>([]);
   ...
   */
  @HostBinding() id = `industry-classification-category-input-${IndustryClassificationCategoryInputComponent._nextId++}`;
  ...
  /**
   * The industry classification category options.
   */
  private _options: IndustryClassificationCategory[] = [];

  /**
   * The code for the system to retrieve the industry classification categories for.
   */
  ...
  /**
   * The code for the selected industry classification category.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected industry classification category.
   *
   * @return The code for the selected industry classification category.
   */
  ...
  /**
   * Set the code for the selected industry classification category.
   *
   * @param value the code for the selected industry classification category
   */
  @Input()
  ...
  displayWith(industryClassificationCategory: IndustryClassificationCategory): string {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.system$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getIndustryClassificationCategories().pipe(first()).subscribe((industryClassificationCategories: Map<string, IndustryClassificationCategory>) => {
        ...
        for (const industryClassificationCategory of industryClassificationCategories.values()) {
          if ((!parameters.system) || (industryClassificationCategory.system === parameters.system)) {
            this._options.push(industryClassificationCategory);
          }
        }
    ...
    this.subscriptions.add(this.inputValue$.pipe(
      ...
      let filteredOptions: IndustryClassificationCategory[] = [];
