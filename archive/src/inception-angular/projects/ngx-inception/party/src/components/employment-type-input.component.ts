...
export class EmploymentTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  filteredOptions$: BehaviorSubject<EmploymentType[]> = new BehaviorSubject<EmploymentType[]>([]);
  ...
  /**
   * The code for the employment status to retrieve the employment types for.
   */
  private employmentStatus$: BehaviorSubject<string | null> = new BehaviorSubject<string | null>(null);
   ...
   * The code for the employment status to retrieve the employment types for.
   */
  @Input() get employmentStatus(): string | null {
    return this.employmentStatus$.value;
  }

  set employmentStatus(employmentStatus: string | null) {
    if (employmentStatus == undefined) {
      employmentStatus = null;
    }

    if (employmentStatus !== this.employmentStatus$.value) {
      this.employmentStatus$.next(employmentStatus);
    }
  }
  ...
  displayWith(employmentType: EmploymentType): string {
    if (!!employmentType) {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(combineLatest([this.employmentStatus$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getEmploymentTypes().pipe(first()).subscribe((employmentTypes: Map<string, EmploymentType>) => {
        ...
        for (const employmentType of employmentTypes.values()) {
          if ((!parameters.employmentStatus) || (employmentType.employmentStatus === parameters.employmentStatus)) {
            this._options.push(employmentType);
          }
        }
