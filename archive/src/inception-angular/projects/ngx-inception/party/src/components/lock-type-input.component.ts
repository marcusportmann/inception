...
import {LockType} from '../services/lock-type';
import {PartyReferenceService} from '../services/party-reference.service';

/**
 * The LockTypeInputComponent class implements the lock type input component.
 *
 * @author Marcus Portmann
 */
@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'lock-type-input',
  template: `
    <div matAutocompleteOrigin #origin="matAutocompleteOrigin">
      <input
        ...
        required="required"
        [matAutocomplete]="lockTypeAutocomplete"
        [matAutocompleteConnectedTo]="origin"
        (input)="inputChanged($event)"
        (focusin)="onFocusIn($event)"
        (focusout)="onFocusOut($event)">
      <mat-autocomplete
        #lockTypeAutocomplete="matAutocomplete"
        (closed)="onClosed()"
  ...
  providers: [
    {
      ...
      useExisting: LockTypeInputComponent
    }
  ]
})
export class LockTypeInputComponent implements MatFormFieldControl<string>,
   ...
   */
  controlType = 'lock-type-input';

  /**
   * The filtered options for the autocomplete.
   */
  filteredOptions$: BehaviorSubject<LockType[]> = new BehaviorSubject<LockType[]>([]);
  ...
  /**
   * The lock type options.
   */
  private _options: LockType[] = [];

  /**
   * The code for the lock type category to retrieve the lock types for.
   */
  private category$: BehaviorSubject<string | null> = new BehaviorSubject<string | null>(null);

  /**
   * The code for the party type to retrieve the lock types for.
   */
  ...
  /**
   * The code for the selected lock type.
   */
  private _value: string | null = null;

  /**
   * Returns the code for the selected lock type.
   *
   * @return The code for the selected lock type.
   */
  ...
  /**
   * Set the code for the selected lock type.
   *
   * @param value the code for the selected lock type
   */
  @Input()
    ...
    this.subscriptions.add(combineLatest([this.category$, this.partyType$]).pipe(throttleTime(250), map(values => ({
      ...
      this.partyReferenceService.getLockTypes().pipe(first()).subscribe((lockTypes: Map<string, LockType>) => {
        ...
        for (const lockType of lockTypes.values()) {
          if (((!parameters.category) || (lockType.category === parameters.category)) &&
            ((!parameters.partyType) || ((!!lockType.partyTypes) && (lockType.partyTypes.indexOf(parameters.partyType) !== -1)))) {
            this._options.push(lockType);
          }
        }
