...
export class SessionService {
  ...
  private static createSessionFromAccessToken(accessToken: string, refreshToken: string | undefined): Session {
    ...
    return new Session((!!token.user_name) ? token.user_name : '', (!!token.user_directory_id) ? token.user_directory_id : '',
      (!!token.user_full_name) ? token.user_full_name : '', (!!token.scope) ? token.scope : [],
      (!!token.authorities) ? token.authorities : [], accessToken, (!!accessTokenExpiry) ? accessTokenExpiry : undefined, refreshToken);
  }
