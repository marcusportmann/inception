...
9. Edit the *src/main/angular/angular.json* file and change the *outputPath* option to *../../../target/frontend* as shown below:
   ...
   ```
10. Execute the following command under the *src/main/angular* directory to import the *ngx-inception* library:
    ```
...
### Create the top-level project for the new application

1. Select the name for the new application, e.g. demo.
2. Create the top-level directory for the new application e.g. demo.
3. Execute the following commands under the top-level directory for the new application to create the project directory structure:

   ```
   mkdir -p src/backend
   mkdir -p src/frontend
   ```
3. Create the *pom.xml* file under the *src* directory with the following contents, changing the *groupId*, *artifactId*, *version*, *name* and *description* details as appropriate:
   ```
   <?xml version="1.0" encoding="UTF-8"?>
   <project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
     xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd">
...
### Create the back-end sub-project for the new application

1. Execute the following commands under the top-level directory for the new application to create the back-end subproject directory structure:
   ```
   mkdir -p src/backend/src/main/java
   mkdir -p src/backend/src/main/java-templates
   mkdir -p src/backend/src/main/resources/META-INF
   mkdir -p src/backend/src/test/java
   mkdir -p src/backend/src/test/resources
   mkdir -p src/frontend
   ```
2. Execute the following commands under the top-level directory for the new application to generate the RSA keypair used to sign and verify OAuth2 JWTs:
   ```
   openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -pkeyopt rsa_keygen_pubexp:65537 | openssl pkcs8 -topk8 -nocrypt -outform pem > src/backend/src/main/resources/META-INF/oauth2-jwt-key
   openssl pkey -pubout -inform pem -outform pem -in src/backend/src/main/resources/META-INF/oauth2-jwt-key -out src/backend/src/main/resources/META-INF/oauth2-jwt-key.pub
   ```

3. Create the *pom.xml* file under the *src/backend* directory with the following contents,
   changing the *artifactId*, *name*, *description* and *parent* details as appropriate.
   The *parent* details should match those specified in the **Create the top-level project for the new application** section above:
   ```
   <?xml version="1.0" encoding="UTF-8"?>
   <project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
     xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd">
   ...
   ```
4. Add the application class, similar to the one shown below, to the backend sub-project:
   ```
   package digital.inception.demo;
   ...
   ```
5. Add the Spring application configuration file, *backend/src/main/resources/application.yml*, to the backend sub-project changing the *spring.application.name* value:
   ```
   inception:
     application:
       data-source:
...
### Create the front-end sub-project for the new application

1. Execute the following commands under the top-level directory for the new application to create the front-end subproject directory structure:
   ```
   mkdir -p src/frontend
   ```
